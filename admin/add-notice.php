<?php
// Admin page for creating notice board entries.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($errorMessage === '') {
    ...
    $insertSql = 'INSERT INTO notices (title, message, target_audience, class_id, publish_date, expiry_date, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    $insertStmt = $connection->prepare($insertSql);
    if (!$insertStmt) {
      $errorMessage = 'Unable to save notice right now.';
    } else {
  ...
  if ($errorMessage === '') {
    admin_set_flash('success', 'Notice published successfully.');
...
<body>
  ...
  <div class="main-content">
    <div class="header">
      <h2><i class="fas fa-plus-circle"></i> Add Notice</h2>
    </div>
    ...
    <div class="form-card">
      ...
      <form method="POST" action="" novalidate>
        ...
        <div class="mt-2">
          <button type="submit" class="btn-submit"><i class="fas fa-bullhorn"></i> Publish Notice</button>
